const apply1 = (f: (input: number) => number): number => f(1);
// const apply1 = (f: Function): any => f(1);
...
console.log(`apply1(a => a - 1): ${apply1((a: number) => a - 1)}`);

// times 함수는 f 라는 함수는 n 번 실행하는 함수
const times = (f: (input: number) => any, n: number) => {
    let i = -1;
    while (++i < n) {
        f(i);
    }
...
// const addMaker = (a: number) => (b: number) => {
//     console.log(`a: ${a}, b: ${b}`);
//     return a + b;
// };
